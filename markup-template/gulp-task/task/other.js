module.exports = function() {
	...
	$.gulp.task('video:build', function() {
		return $.gulp.src($.config.pathVar.path.src.video)
			.pipe($.gp.plumber({ errorHandler: $.gp.notify.onError("Error: <%= error.message %>") }))
			.pipe($.gulp.dest($.config.pathVar.path.build.video))
	});
	...
	//other file build version project
	$.gulp.task('video:buildProd', function() {
		return $.gulp.src($.config.pathVar.path.src.video)
			.pipe($.gp.plumber({ errorHandler: $.gp.notify.onError("Error: <%= error.message %>") }))
			.pipe($.gulp.dest($.config.pathVar.path.production.video))
	});
